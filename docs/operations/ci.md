...
- **CI disables Xcode's debug dylib sidecar.** Xcode 16 Debug builds can emit a `LocalTypeless.debug.dylib` sidecar; hardened runtime library validation rejects that sidecar on user machines. CI passes `ENABLE_DEBUG_DYLIB=NO`, then signs the resulting `.app` separately.
